...
# FINAL, GORGEOUS, FLAWLESS DESIGN
st.markdown("""
...
<style>
    ...
    .card:hover {transform: translateY(-12px); box-shadow: 0 30px 70px rgba(0, 210, 255, 0.4);}
    ...
    .result-metric {
        ...
        box-shadow: 0 10px 30px rgba(58, 125, 213, 0.5);
    }
    ...
    /* UPLOAD BUTTONS — BEAUTIFUL & WORKING */
    .stFileUploader > div > div > label {color: #00d2ff !important; font-weight: 700; font-size: 1.3rem;}
    .stFileUploader > div > div > div {background: rgba(30, 30, 60, 0.8) !important; border: 3px dashed #00d2ff !important; border-radius: 20px; padding: 2rem !important;}
    
    /* SIDEBAR — ALIVE WITH ICONS */
    .css-1d391kg [data-testid="stSidebarNav"] {padding-top: 2rem;}
    .css-1d391kg [data-testid="stSidebarNav"] a {
        color: #b0b0ff !important; font-size: 1.1rem; padding: 15px 20px !important;
        border-radius: 16px; margin: 8px 12px; transition: all 0.3s;
    }
    .css-1d391kg [data-testid="stSidebarNav"] a:hover {
        background: rgba(0, 210, 255, 0.3) !important; color: white !important;
        transform: translateX(10px);
    }
    
    h1, h2, h3, h4, p, .stMarkdown {color: #e0e0ff !important;}
</style>
""", unsafe_allow_html=True)

# SIDEBAR — NOW ALIVE WITH ICONS
st.sidebar.markdown("""
    ...
    for i, (title, icon, color) in enumerate(features):
        with cols[i]:
            ...
            <div class='feature-card'>
                ...
                <h4 style='color:#e0e0ff; margin-top:1.5rem; line-height:1.4; font-size:1.1rem;'>{title}</h4>
            </div>
            """, unsafe_allow_html=True)
...
# ABOUT ME PAGE
    ...
    with col1:
        st.image("https://avatars.githubusercontent.com/u/123456789?v=4", width=300)
    with col2:
        st.markdown("""
        <h2 style='color:#00d2ff;'>Aspiring AI Engineer • Bengaluru</h2>
        <p style='color:#e0e0ff; font-size:1.3rem; line-height:1.9;'>
