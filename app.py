...
# MUST BE FIRST — fixes all Streamlit errors
st.set_page_config(
...
<style>
    ...
    .subtitle {color: #b0b0ff; font-size: 1.5rem; font-weight: 400; margin-top: 1rem;}
    ...
    /* Feature cards - full height, no empty space */
    .feature-card {
    ...
    .feature-card:hover {background: rgba(59, 130, 246, 0.2); transform: translateY(-8px);}
    ...
    /* Upload buttons - fully visible & beautiful */
    ...
    .sidebar .sidebar-content {
        background: linear-gradient(to bottom, #1a1a3a, #0f0f1a);
        color: white;
    }
    
    /* Text visibility */
    h1, h2, h3, h4, p, li, .stMarkdown {color: #e0e0ff !important;}
</style>
...
# ============ FEATURES PAGE — FIXED (NO EMPTY CARDS!) ============
elif page == "Features":
    st.markdown("<div class='header'>", unsafe_allow_html=True)
    ...
    for i, (title, icon, color) in enumerate(features):
        with cols[i]:
            ...
            <div class='feature-card'>
                <h2><i class='{icon}' style='color:{color}; font-size:4rem; margin-bottom:1rem;'></i></h2>
                <h4 style='color:#e0e0ff; margin:0;'>{title}</h4>
            </div>
            """, unsafe_allow_html=True)
...
# ============ ABOUT ME PAGE ============
    ...
    with col1:
        st.image("https://avatars.githubusercontent.com/u/123456789?v=4", width=280, caption="AI Developer")
    with col2:
        st.markdown("""
